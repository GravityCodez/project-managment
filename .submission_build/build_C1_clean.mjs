...
],[2600000,5300000,3504599],1,'One-way audio delay target: 95th percentile at most 150 ms.',17);
...
fourRows(c.slides[2],'Authority and open decisions',[
 ...
 ['Resources','100 student hours: Temiko 54 and Nurtore 46. AED 0 incremental cash through reuse of existing equipment.'],
 ['To confirm','Course sponsor and authorization, equipment access, volunteer availability and the proposed 100-hour team capacity.']
...
const decks=[
 ...
 'Sources: ClassMic aspiration, business case and charter. The targets cover scope completion, usable setup and speaking, perceived audibility, and instructor control. Ready-speaker trials measure time from instructor approval to audible speech. Audio transport delay is measured separately, with median and 95th percentile reported and a proposed 95th percentile target of 150 milliseconds.',
 'Source: revised C1 charter. The course scope contains five priority stories and two one-week sprints. The earlier 14-week classroom pilot is outside the current course authorization. The proposed schedule places planning review by 19 September, followed by the two build sprints and a closeout review by 9 October.',
 'Sources: ClassMic charter and business case. Nurtore coordinates the project and Temiko leads technical development. Proposed effort is 54 hours for Temiko and 46 for Nurtore. Course authorization, the named sponsor and access to equipment and reviewers remain open. The instructor reviews material scope or schedule changes.'
 ]}
];
...
for(const item of decks.filter(d => d.key === 'C1')){
 ...
 const candidatePath=path.join(build,item.key+'_clean_candidate.pptx');
 const finalPath=path.join(build,'revised_slides',`ClassMic_${item.key}_Three_Slides_clean.pptx`);
 ...
 await finalizePresentation({workspaceDir:root,candidatePath,finalPath,
  ...
  verifyArtifactToolImport:true,receiptPath:path.join(build,item.key+'_clean_validation.json')});
 ...
 for(let i=0;i<3;i++){
  ...
  await fs.writeFile(path.join(build,`${item.key}_clean_slide_${i+1}.png`),new Uint8Array(await png.arrayBuffer()));
 }
