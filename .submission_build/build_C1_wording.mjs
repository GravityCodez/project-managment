...
const decks=[
 ...
 'Source: revised C1 charter. Formal course acceptance, the named sponsor and resource access require confirmation. The package includes Temiko’s original SMART note and a charter planning worksheet. An original image of the team discussing the charter remains to be supplied.'
 ]}
];
...
for(const item of decks.filter(d => d.key === 'C1')){
 ...
 const candidatePath=path.join(build,item.key+'_wording_candidate.pptx');
 const finalPath=path.join(build,'revised_slides',`ClassMic_${item.key}_Three_Slides_final.pptx`);
 ...
 await finalizePresentation({workspaceDir:root,candidatePath,finalPath,
  ...
  verifyArtifactToolImport:true,receiptPath:path.join(build,item.key+'_wording_validation.json')});
 ...
 for(let i=0;i<3;i++){
  ...
  await fs.writeFile(path.join(build,`${item.key}_wording_slide_${i+1}.png`),new Uint8Array(await png.arrayBuffer()));
 }
